  ...
  return (sourceFile: string, module: IReplacerModule) => {
    ...
    if (moduleRoot.indexOf('node_modules') > 0) {
      try {
        ...
        let moduleTsdFile = pkg.module || pkg.exnext || pkg.typings
        if (moduleTsdFile) {
...
export function replacer(
  sourceFile: string,
  contentOrModules: string | IReplacerModule[],
  modules?: IReplacerModule[]
): IReplacerResult {
...
function replace(
  this: IReplacerModule,
  refModules: string[],
  djson: IDts2djsonResult,
  raw: string,
  preSpaces: string,
  inOut: string,
  rawimports: string,
  ...
  interface IAnalyzeResult {
    variables: { [file: string]: IVariable[] }
    files: string[]
  }

  // 先分析出要导出或导入的变量，方便后面的批量操作
  const result: IAnalyzeResult = { variables: {}, files: [] }
  stripInlineComment(rawimports.trim())
    .split(splitRegexp)
    .reduce((res: IAnalyzeResult, field: string) => {
  ...
  // 再来一个个替换
  const rawReplace = result.files
    .reduce((lines: string[], file: string) => {
      // 兼容 window 换行符
