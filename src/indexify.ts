...
import { capitalize } from './inc/helper'
...
export function indexify(folder: string, options: IIndexifyOptions = {}): string {
  ...
  let filter = (stats: fs.Stats, name: string, absolutePath: string) =>
    optionFilter(stats, name, getRelativePath(absolutePath), absolutePath)
  let opts: IInnerOptions = {
    rename,
    deep: 0,
    ...options,
    filter,
