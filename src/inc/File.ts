...
export class File {
  ...
  constructor(src: string, content?: string) {
    this.src = src
    this.content = (content == null ? fs.readFileSync(src).toString() : content).replace(
      config.exportNamespaceRegExp,
      ''
    ) // 清除 namespace 的定义
  }
  ...
  private _addImportKey(
    key: string,
    obj: IFileImportKey,
    options: IFileCompileOptions,
    checkIfIsReallyDefault?: boolean
  ) {
    let from = this.findReferFile(obj.from)
    if (!from) {
      // 如果是相对目录时，不应该解析失败！
      // 绝对目录可能是 node_modules 下的目录
      if (obj.from.charAt(0) === '.' && /\.[jt]s$/.test(obj.from)) {
  ...
  private _analyzeImportLocals(locals: string, from: string, options: IFileCompileOptions) {
    locals = locals.trim()
    if (!locals) return
    locals.split(config.splitRegexp).forEach(field => {
  ...
  private findReferFile(ref: string, tryDirectory = true): string | null {
    if (ref[ref.length - 1] === '/') {
      tryDirectory = false
    ...
    return fn.isFileExists(dtsFile)
      ? dtsFile
      : fn.isFileExists(tsFile)
      ? tsFile
      : fn.isFileExists(tsxFile)
      ? tsxFile
      : tryDirectory
      ? this.findReferFile(ref + '/', false)
      : null
  }
}
